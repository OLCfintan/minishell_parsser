...
void	check_cmd(t_cmd **cmd_list, t_cmd **curr_cmd, char **env)
{
	*cmd_list = init_cmd(env);
	*curr_cmd = *cmd_list;
}

void	collect_cmd(t_cmd *curr_cmd, t_lexer *lexer, t_token *token, char **env)
{
	if (token->type == CMD)
...
t_cmd	*parse_input(t_lexer *lexer, char **env)
{
	...
	{
		if (!cmd_list)
			check_cmd(&cmd_list, &curr_cmd, env);
		collect_cmd(curr_cmd, lexer, token, env);
		free_token(token);
		token = lexer_get_next_token(lexer, env);
	}
	free_token(token);
